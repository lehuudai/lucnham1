...
def tinh_tam_truyen_chi_tiet(can_ngay, chi_ngay, thien_ban, tu_khoa):
    ...
    # Khóa 1 (Thiên Bàn vs Can Ngày)
    if la_khac(can_ngay, k1_t): tac_list.append((k1_t, k1_d))
    if la_khac(k1_t, can_ngay): khac_list.append((k1_t, k1_d))
    
    # Khóa 2, 3, 4 (Thiên Bàn vs Địa Bàn bên dưới)
    for t, d in [(k2_t, k2_d), (k3_t, k3_d), (k4_t, k4_d)]:
        if la_khac(d, t): tac_list.append((t, d))
        if la_khac(t, d): khac_list.append((t, d))
    ...
    if cand_list:
        if len(cand_list) == 1:
            ...
            if is_phuc_ngam: ten_phap = "Phục Ngâm (Tự Nhậm)"
        else:
            ...
            if len(ty_list) == 1:
                ...
                # [LỖI ĐÃ FIX 1] Xác định ứng viên từ Tặc (Dưới khắc Trên) hay Khắc (Trên khắc Dưới)
                is_tac = len(tac_list) > 0 
                                
                depth_uv = []
                for uv in ung_vien:
                    t_chi, d_chi = uv[0], uv[1]
                    start_idx = DIA_CHI.index(d_chi) 
                    end_idx = DIA_CHI.index(t_chi)   
                    ...
                    for i in range(steps + 1):
                        ...
                        # Nếu là Tặc -> Đếm dọc đường có bao nhiêu cung chém nó
                        if is_tac: 
                            if la_khac(cung_di_qua, t_chi): so_lan_day_vo += 1
                        # Nếu là Khắc -> Đếm dọc đường có bao nhiêu cung bị nó chém
                        else:      
        ...
        # [LỖI ĐÃ FIX 2] Xử lý truyền Hình cho Phục Ngâm có Khắc (Ngăn ngừa lỗi ra A, A, A)
        if is_phuc_ngam:
            if so in ["Thìn", "Ngọ", "Dậu", "Hợi"]:
                trung = XUNG_MAP[so]
                mat = get_hinh(trung)
            else:
                trung = get_hinh(so)
