...
def quet_100_tat_phap_phu(tu_khoa, tam_truyen, khong_vong_ngay, can_ngay, thien_dia_ban=None, ban_menh_data=None, vi_mo_data=None, tu_tru=None):
   ...
   # ========================================================
    ...
    chi_tam_truyen = {so_chi, trung_chi, mat_chi}
    
    # 1. Xác định Ngũ Hành của Cục Diện
    cuc_hanh = None
    if "Tam Hợp Cục" in tam_truyen_cuc_dien:
        if {"Dần", "Ngọ", "Tuất"}.issubset(chi_tam_truyen): cuc_hanh = "Hỏa"
        elif {"Hợi", "Mão", "Mùi"}.issubset(chi_tam_truyen): cuc_hanh = "Mộc"
        elif {"Thân", "Tý", "Thìn"}.issubset(chi_tam_truyen): cuc_hanh = "Thủy"
        elif {"Tỵ", "Dậu", "Sửu"}.issubset(chi_tam_truyen): cuc_hanh = "Kim"
    
    # 2. Kiểm tra xem cái Cục Ngũ Hành khổng lồ đó có phải là Thê Tài của Can Ngày không
    is_tai_cuc = False
    if cuc_hanh:
        is_tai_cuc = la_khac(NGU_HANH.get(can_ngay), cuc_hanh)
